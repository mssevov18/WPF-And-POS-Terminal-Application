...
char myKeypadkeys[numRows][numCols] =
{
  {'1', '2', '3', 'A'},
  {'4', '5', '6', 'B'},
  {'7', '8', '9', 'C'},
...
Keypad myKeypad = Keypad(makeKeymap(myKeypadkeys), rowPins, colPins, numRows, numCols);
...
void loop()
{
  if (menuOption == '1')
  {
    // Membrane 4x4 Matrix Keypad - Test Code
    //Read keypad
    char myKeypadKey = myKeypad.getKey();
    Serial.print(myKeypadKey);
  }
...
char menu()
{
  ...
  Serial.println(F("(1) Membrane 4x4 Matrix Keypad"));
  ...
  while (Serial.available())
  {
    ...
    if (isAlphaNumeric(c))
    {
      ...
      if (c == '1')
        Serial.println(F("Now Testing Membrane 4x4 Matrix Keypad"));
      else if (c == '2')
